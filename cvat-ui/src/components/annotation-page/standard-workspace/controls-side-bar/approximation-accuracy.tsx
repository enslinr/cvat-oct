...
function ApproximationAccuracy(props: Props): React.ReactPortal | null {
    ...
    // Calculate the tolerance value for display
    const tolerance = thresholdFromAccuracy(approxPolyAccuracy);
    const toleranceDisplay = tolerance === 0 ? 'exact' : tolerance.toFixed(2);

    return target ?
        ReactDOM.createPortal(
            <Row align='middle' className='cvat-approx-poly-threshold-wrapper'>
                <Col span={8}>
                    <Text>Points ({toleranceDisplay}): </Text>
                </Col>
                <Col offset={1} span={15}>
                    <Slider
                        ...
                        tooltip={{
                            formatter: (value) => {
                                const tol = thresholdFromAccuracy(value ?? 0);
                                return tol === 0 ? 'Exact (no simplification)' : `Tolerance: ${tol.toFixed(2)}px`;
                            },
                        }}
                        onChange={onChange}
                        marks={marks}
