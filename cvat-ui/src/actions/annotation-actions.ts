...
export function createAnnotationsAsync(statesToCreate: any[]): ThunkAction {
    return async (dispatch: ThunkDispatch): Promise<any[]> => {
        console.log('[SAM2-OCT] createAnnotationsAsync called with', statesToCreate.length, 'states');
        try {
            const { jobInstance } = receiveAnnotationsParameters();
            const clientIDs = await jobInstance.annotations.put(statesToCreate);
            console.log('[SAM2-OCT] jobInstance.annotations.put completed successfully. IDs:', clientIDs);
            dispatch(fetchAnnotationsAsync());
            return clientIDs;
        } catch (error) {
            console.error('[SAM2-OCT] createAnnotationsAsync failed:', error);
            dispatch({
