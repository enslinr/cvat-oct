...
import { getSharedBoundaryHandler, SharedBoundaryHandler } from './sharedBoundaryHandler';
...
export class CanvasViewImpl implements CanvasView, Listener {
    ...
    private sharedBoundaryHandler: SharedBoundaryHandler;
    ...
    private markSharedPoints(clientID: number, shape: SVG.Shape): void {
        // Get the select handler which contains the nested point circles
        const handler = shape.remember('_selectHandler');
        if (!handler || !handler.nested) return;

        const nested = handler.nested;
        const children = nested.children();

        // Iterate through the point circles and mark shared ones
        for (let pointIndex = 0; pointIndex < children.length; pointIndex++) {
            const pointCircle = children[pointIndex];
            if (pointCircle && pointCircle.type === 'circle') {
                if (this.sharedBoundaryHandler.isSharedPoint(clientID, pointIndex)) {
                    pointCircle.addClass('cvat_canvas_shape_shared_point');
                } else {
                    pointCircle.removeClass('cvat_canvas_shape_shared_point');
                }
            }
        }
    }

    private propagateLinkedPointEdits(state: any, newPoints: number[]): void {
        // Get the old points from drawn states for comparison
        const drawnState = this.drawnStates[state.clientID];
        if (!drawnState || drawnState.shapeType !== 'polygon') return;

        const oldPoints = drawnState.points;
        if (!oldPoints || oldPoints.length !== newPoints.length) return;

        // Track which polygons need updates and their new points
        const updates: Map<number, number[]> = new Map();

        // Check each point for movement
        for (let i = 0; i < newPoints.length; i += 2) {
            const oldX = oldPoints[i];
            const oldY = oldPoints[i + 1];
            const newX = newPoints[i];
            const newY = newPoints[i + 1];

            // Check if this point moved
            if (Math.abs(oldX - newX) > 0.001 || Math.abs(oldY - newY) > 0.001) {
                const pointIndex = i / 2;
                const linkedPoints = this.sharedBoundaryHandler.getLinkedPoints(state.clientID, pointIndex);

                for (const linked of linkedPoints) {
                    // Get or initialize the points array for this linked polygon
                    if (!updates.has(linked.polygonClientID)) {
                        const linkedDrawnState = this.drawnStates[linked.polygonClientID];
                        if (linkedDrawnState && linkedDrawnState.points) {
                            updates.set(linked.polygonClientID, [...linkedDrawnState.points]);
                        }
                    }

                    const linkedPoints2 = updates.get(linked.polygonClientID);
                    if (linkedPoints2) {
                        // Update the linked point to match the new position
                        linkedPoints2[linked.pointIndex * 2] = newX;
                        linkedPoints2[linked.pointIndex * 2 + 1] = newY;
                    }
                }
            }
        }

        // Dispatch edit events for all linked polygons that need updates
        if (updates.size > 0) {
            console.log(`[SharedBoundary] Propagating point edits to ${updates.size} linked polygon(s)`);
        }

        for (const [clientID, updatedPoints] of updates) {
            const linkedState = this.controller.objects.find(
                (s: any) => s.clientID === clientID,
            );
            if (linkedState) {
                // Mark the drawn state for update
                this.drawnStates[clientID].updated = 0;
                this.drawnStates[clientID].points = [];

                // Dispatch the edit event for the linked polygon
                this.canvas.dispatchEvent(
    ...
    private onEditDone = (state: any, points: number[], rotation?: number): void => {
        ...
        this.mode = Mode.IDLE;
        if (state && points) {
            // Propagate edits to linked polygon points (shared boundaries)
            if (state.shapeType === 'polygon') {
                this.propagateLinkedPointEdits(state, points);
            }
    ...
    private setupObjects(states: any[]): void {
        ...
        // Analyze shared boundaries for polygon linking
        const polygonStates = states.filter(
            (state: any) => state.shapeType === 'polygon' && !state.hidden && !state.outside,
        );
        if (polygonStates.length >= 2) {
            this.sharedBoundaryHandler.analyzeSharedPoints(polygonStates, 1.0);
        }
    ...
    public constructor(model: CanvasModel & Master, controller: CanvasController) {
        ...
        // Initialize shared boundary handler for linked polygon editing
        this.sharedBoundaryHandler = getSharedBoundaryHandler();
    ...
    private activateShape(clientID: number): void {
        ...
        // Mark shared boundary points with visual indicator
        if (state.shapeType === 'polygon') {
            this.markSharedPoints(clientID, shape);
        }
