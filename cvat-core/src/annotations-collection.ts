...
export default class Collection {
    ...
    public put(objectStates: ObjectState[]): number[] {
        ...
        console.log('[SAM2-OCT] Collection.put called with', objectStates.length, 'objects');
        for (const state of objectStates) {
            const stateIndex = objectStates.indexOf(state);
            console.log(`[SAM2-OCT] Processing object ${stateIndex + 1}/${objectStates.length}:`, {
        ...
        console.log('[SAM2-OCT] Imported objects:', {
            tags: imported.tags.length,
            tracks: imported.tracks.length,
            shapes: imported.shapes.length,
            totalImported: importedArray.length,
            shapeDetails: imported.shapes.map((s, i) => ({
                index: i,
                clientID: s.clientID,
