...
class SAM2ImagePredictor:
    ...
    def set_image(
        ...
        if isinstance(image, Image):
            ...
            # Ensure it's RGB (convert if grayscale)
        ...
        # Pad the image
        image_padded = self._pad_sample_numpy(image_np)

        # Convert to tensor
        input_image = torch.from_numpy(image_padded).permute(2, 0, 1).float() / 255.0
        ...
        self._features = {"fused_features": fused_features, "high_res_feats": high_res_features}
    ...
    def _predict(
        ...
        if self._features is None:
            raise RuntimeError("Features are not set.")

        # Use semantic SAM2 forward for multi-class prediction (no prompts needed)
        # Get high res features
        if len(self._features["high_res_feats"]) > 0:
            high_res_features = [
        ...
        # Get fused features (backbone features)
        fused_features = self._features["fused_features"]
        high_res_features = self._features["high_res_feats"]
        # Call _forward_sam_heads directly for semantic segmentation
        ...
        # Unpack outputs: (low_res_multimasks, high_res_multimasks, ious, low_res_masks, high_res_masks, obj_ptr, object_score_logits)
        _, high_res_multimasks, _, _, _, _, _ = sam_outputs

        # high_res_multimasks shape: [batch_size, num_classes, H, W]
        masks = high_res_multimasks

        # Create dummy IOU predictions (num_classes,)
        num_classes = masks.size(1)
        iou_predictions = torch.ones(1, num_classes, device=masks.device)

        # Create low res masks by downsampling
